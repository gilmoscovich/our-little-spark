...
export const RoomView = ({
  ...
  const accent =
    room.level === "spicy" ? "bg-gradient-spicy" : "bg-gradient-primary";
  ...
  const handleModeChange = async (mode: Mode) => {
    await updateRoomModeLevel(room, mode, room.level);
  };

  const handleLevelChange = async (level: Level) => {
    await updateRoomModeLevel(room, room.mode, level);
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto flex flex-col items-center gap-5">
      <div className="w-full flex items-center justify-between">
        ...
        <button
          onClick={copyCode}
          className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/70 border border-white/80 shadow-soft text-sm font-mono tracking-[0.3em]"
        >
          {room.code}
      ...
      {!partnerJoined && (
        <div className="w-full text-center text-sm text-muted-foreground bg-white/50 rounded-2xl px-4 py-3 border border-white/80">
          Share the code <span className="font-mono font-semibold text-foreground">{room.code}</span> with your partner to start playing.
        </div>
      )}
      ...
      <div className="text-center">
        ...
        <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground mt-1">
          {isMyTurn ? "Your turn — read & answer" : "Partner's turn"}
        </p>
      </div>

      <div
        key={room.current_prompt ?? "empty"}
      ...
      <Button
        ...
        disabled={!isMyTurn}
        size="lg"
        className="bg-gradient-primary hover:opacity-90 text-primary-foreground rounded-full px-8 shadow-soft disabled:opacity-50"
      >
        <Shuffle className="w-4 h-4 mr-2" />
        {isMyTurn ? "Send next prompt" : "Waiting for partner…"}
      </Button>
    </div>
  );
};
